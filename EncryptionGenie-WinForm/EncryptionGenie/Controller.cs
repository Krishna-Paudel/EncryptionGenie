...
namespace EncryptionGenie
{
    ...
    public class Controller
    {
        ...
        public Controller()
        {
            Log.Info("New instance of Controller has been created.");
        ...
        public byte[] GenerateMasterKey(string password, int KeySize, byte[] salt, int Iterations, string hashAlgo)
        {
            ...
            using (var pbkdf2 = new Rfc2898DeriveBytes(password, salt, Iterations, hashAlgorithmName))
            {
                ...
                stopwatch.Stop();
                Log.Info("Iteration = " + Iterations + "   TotalTime (in ms) = " + stopwatch.ElapsedMilliseconds + "   TotalTime (in Ticks) = " + stopwatch.ElapsedTicks);

                byte[] masterKey = new byte[SaltSize + KeySize];

                // Concatenate the salt and key into the master key
                Array.Copy(salt, 0, masterKey, 0, SaltSize);
                Array.Copy(key, 0, masterKey, SaltSize, KeySize);                

                return masterKey;
            }
        }

        // function to capture benchmarking of key generation
        public void benchmarkMasterKeyGen(string EncAlgo, string HashAlgo, string SecretText)
        {
            
            for (int i = 0; i < 100; i++)
            {
                int Iterations = new Random().Next(100, 1000000);
                
                int encKeySize = FindKeySizeInBytes(EncAlgo);
                int hmacKeySize = FindKeySizeInBytes(HashAlgo);

                byte[] masterSaltBytes = RandomNumberGenerator.GetBytes(SaltSize);
        ...
        public async Task<string> encryptFile(string FilePath, string EncAlgo, string HashAlgo, string SecretText, ProgressForm progressForm)
        {

            Log.Info("Encryption of file " + FilePath + "has started.");
            ...
            await Task.Run(() =>
            {
                try
                {
                    ...
                    Log.Info("Master Key, Encryption Key and HMAC Key for encryption process has been generated.");

                    // Generate IV of one block size
                    int blockSize = FindBlockSizeInBytes(EncAlgo);
                    byte[] iv = RandomNumberGenerator.GetBytes(blockSize);

                    progressForm.UpdateEncProgress("Loading file to encrypt...");
                    ...
                    using (SymmetricAlgorithm encAlgo = CreateCipher(EncAlgo))
                    {
                        ...
                        progressForm.UpdateEncProgress("Encryption in progress...");
                    ...
                    Log.Info("Encryption completed. Generating metadata.");
                    ...
                    progressForm.UpdateEncProgress("Encryption Completed. Generating hash...");
                    ...
                    progressForm.UpdateEncProgress("Generating encrypted file...");
            ...
            Log.Info("Generated the ecnrypted file at " + encryptedFilePath);
        ...
        public async Task<string> decryptFile(string encFilePath, string SecretText, ProgressForm progressForm)
        {
            ...
            await Task.Run(() =>
            {
                try
                {
                    Log.Info("Decryption of file " + encFilePath + " has started.");


                    using (BinaryReader reader = new BinaryReader(File.Open(encFilePath, FileMode.Open)))
                    {
                        progressForm.UpdateProgress(10);
                        progressForm.UpdateEncProgress("Loading encrypted file into memory.");

                        Log.Info("Loading encrypted file into memory.");
                        ...
                        progressForm.UpdateProgress(20);
                        progressForm.UpdateEncProgress("Extracting information about the file...");
                        ...
                        Log.Info("Fetching metadata from the file.");

                        progressForm.UpdateProgress(30);
                        progressForm.UpdateEncProgress("Regenerating keys to decrypt...");

                        Log.Info("Regenerating Master Key, Encryption Key and HMAC Key.");
                        ...
                        progressForm.UpdateEncProgress("Comparing hashes to ensure file is not tampered...");
                        ...
                        Log.Info("Reading the IV of one block size.");
                        ...
                        // Recompute the hash
                        Log.Info("Computing hash for integrity check.");
                        ...
                        {
                            Log.Info("Hash did not match. The file seems to be altered or corrupted.");
                        ...
                        progressForm.UpdateEncProgress("Decrypting the file....");
                        ...
                        progressForm.UpdateProgress(90);
                        progressForm.UpdateEncProgress("Decryption Completed. Saving file to filesystem...");
                        ...
                        Log.Info("File has been decrypted and save at " + decryptedFilePath);
